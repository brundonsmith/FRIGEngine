	...
	public void init(XMLElement xmlElement) {
		...
		try {
			...
			throw new DataParseException("Failed to load image '"
					+ xmlElement.getAttribute("sprite", "") + "' for animation '" + this.getID()
					+ "'");
		}
		...
		try {
			animation.setLooping(xmlElement.getBooleanAttribute("looping", true));
		} catch (SlickXMLException e) {
		...
		float width;
		try {
			width = (float) xmlElement.getDoubleAttribute("width", sprite.getWidth());
		} catch (SlickXMLException e) {
		...
		float height;
		try {
			height = (float) xmlElement.getDoubleAttribute("height", sprite.getHeight());
		} catch (SlickXMLException e) {
