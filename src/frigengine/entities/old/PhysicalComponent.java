...
public class PhysicalComponent extends EntityComponent {
	...
	public void update(int delta, Input input, Scene scene) {
		if(scene instanceof Area) {
			Area area = (Area)scene;
			if (this.collidable && this.movable) {// If collision should be considered
				// Area collision
				for(Shape shape : area.getBoundaries())
					while (this.getCollisionArea().intersects(shape))
						this.entity.spacial().moveBy(calculateCollisionFix(this.getCollisionArea(), shape));
				
				// Entity collision
				for (Entity otherEntity : scene.getEntities())
					// For each entity,
					if (otherEntity != this.entity // if it's not this entity
							&& otherEntity.hasComponent(PhysicalComponent.class) // and it's physical
							&& otherEntity.physics().collidable) { // and its collision is turned on
						while (this.getCollisionArea().intersects( // If there's an intersection,
								otherEntity.physics().getCollisionArea())) {
							this.entity.spacial().moveBy(calculateCollisionFix(this.getCollisionArea(), otherEntity.physics().getCollisionArea()));
						}
					}
			}
		}
	...
	public Shape getCollisionArea() {
		if (collisionArea instanceof Rectangle)
			return this.collisionArea.transform(Transform.createTranslateTransform(
					this.entity.spacial().getX() - this.collisionArea.getWidth() / 2,
					this.entity.spacial().getY() - this.collisionArea.getHeight() / 2));
		else
			return this.collisionArea.transform(Transform.createTranslateTransform(
					this.entity.spacial().getX(),
					this.entity.spacial().getY()));
	}
